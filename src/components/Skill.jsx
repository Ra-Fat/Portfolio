...
const Skills = () => {
  ...
  return (
    <div  className="flex flex-col items-center justify-center w-full lg:px-0 xl:px-10 ">
      ...
      <div data-aos="fade-up" data-aos-duration="1500" onMouseMove={handleMouseMove} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}
        ...
        <div  className="pointer-events-none absolute w-40 h-40 rounded-full bg-white blur-2xl z-10 transition-opacity duration-300"
          style={{ opacity: isHovering ? 0.1 : 0, transform: `translate(${mousePos.x - 80}px, ${mousePos.y - 80}px)`,}}/>

          {technologies.map(({ name, icon }, idx) => (
            <div key={name}
              className="flex flex-col items-center justify-center p-4 rounded-lg transition-transform duration-300"
              style={{ animation: `bounceSmooth 1.2s ease-in-out infinite`, animationDelay: `${idx * 0.55}s`, }}
            >          
              <img src={icon} alt={name} className="w-12 h-12 object-contain mb-2 brightness-100 transition-all duration-300" draggable={false}/>
