        ...
        {ExperiencesContext.map((exp, index) => (
          <div key={index}
            ...
            className={`w-full relative bg-gray-800/30 p-4 flex items-start gap-10 shadow-inner cursor-pointer rounded-[8px] 
              transition-all duration-300 transform hover:scale-[1.02] hover:shadow-md hover:shadow-gray/10 h-55
          ...
          >
            ...
            <div className="h-full flex justify-center items-center">{exp.year}</div>
            ...
            {expandedIndex === index && (
              <div className="hidden max-[780px]:flex absolute bottom-[45px] left-1/2 -translate-x-1/2 justify-center items-center">
                <div className="relative w-50 h-35 overflow-hidden rounded-md">
                  ...
                  <div className={`absolute inset-0 flex justify-center items-center transition-opacity duration-500 ${expandedIndex === index ? 'opacity-100' : 'opacity-0'}`}>
                    <button onClick={(e) => {  e.stopPropagation();  window.open(exp.pdf, '_blank'); }}  className="bg-white/80 cursor-pointer hover:bg-white p-3 rounded-full shadow-lg transition">
                      <Eye className="w-5 h-5 text-gray-800" />
                    </button>
                  </div>
            ...
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 hidden max-[780px]:flex">
              <ChevronDown className={`transition-transform duration-300 animate-bounce ${ expandedIndex === index ? 'rotate-180' : ''}`} />
            </div>
          </div>
        ))}
      </div>
